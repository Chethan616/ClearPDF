...
fun SplitPdfScreen(
    ...
    Column(
        ...
        Column(
            ...
            BasicText("Split a PDF into separate pages or page ranges", style = TextStyle(sub, 14.sp, textAlign = TextAlign.Center))
        ...
        if (state.sourceFileName.isNotEmpty()) {
            Column(
                ...
                LiquidButton(onClick = { viewModel.onSplit() }, backdrop = backdrop, tint = accent) {
                    Row(horizontalArrangement = Arrangement.spacedBy(6.dp), verticalAlignment = Alignment.CenterVertically) {
                        Icon(Icons.Rounded.ContentCut, null, Modifier.size(18.dp), Color.White)
                        BasicText("Split All Pages", style = TextStyle(Color.White, 15.sp, fontWeight = FontWeight.Medium))
                    }
                }
            }
        }
